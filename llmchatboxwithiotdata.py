# -*- coding: utf-8 -*-
"""LLMChatboxwithIoTData.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/12vzMetppVUIiv4UnxPD5-t_vy5Smo7vI
"""
...
CSV_CLEAN = "/content/smartcity_env_log_cleaned.csv"
...
def load_df():
  ...
  if not os.path.exists(CSV_CLEAN):
    raise FileNotFoundError(f"CSV file not found: {CSV_CLEAN}")
...
def route(df, q: str):
  ...
  - If they ask for "average|avg|mean|min|max|stats|summary" => return stats_over_all
    and choose metric ("temp", "hum", or both) by keyword
  ...
  if any(k in q1 for k in("latest", "current", "now")):
    out = latest_reading(df)
    out["_intent"] = "_latest_reading"
    return out

  # Stats intent (over the whole dataset)
  if any(k in q1 for k in("average", "avg", "mean", "min", "max", "stats", "summary")):
      metric = "both"
      if "temp" in q1:
        metric = "temp"
      elif "hum" in q1 or "humidity" in q1:
        metric = "hum"
